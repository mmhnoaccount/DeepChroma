...
class tag2pix(object):
    def __init__(self, args):
        ...
        self.G = Generator(luma_size=args.input_size, chroma_size=32, ref_length=128,
                           luma_dim=4, output_dim=2, layers=args.layers, net_opt=self.net_opt)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.end_epoch + 1):
            ...
            for iter, (luma_in, chroma_in, ref_in, org_label) in enumerate(tqdm(self.train_data_loader, ncols=80)):
                if iter >= max_iter:
                    break

                if self.gpu_mode:
                    luma_in, chroma_in, ref_in, org_label = luma_in.to(self.device), chroma_in.to(self.device), ref_in.to(self.device), org_label.to(self.device)
                ...
                if iter % 10 == 0:
                    ...
                    G_f, _ = self.G(luma_in, chroma_in, ref_in)
                    if self.gpu_mode:
                ...
                G_f, G_g = self.G(luma_in, chroma_in, ref_in)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for iter, (luma_in, chroma_in, ref_in, org_label) in enumerate(tqdm(self.test_data_loader, ncols=80)):
                if self.gpu_mode:
                    luma_in, chroma_in, ref_in, org_label = luma_in.to(self.device), chroma_in.to(self.device), ref_in.to(self.device), org_label.to(self.device)
                ...
                #G_loss = D_f_fake_loss + (L2_D_f_fake_loss + L2_D_g_fake_loss * self.guide_beta) * self.l2_lambda
                G_loss = (L2_D_f_fake_loss + L2_D_g_fake_loss) * self.l2_lambda
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        luma_in, chroma_in, ref_in, org_label = next(iter(visual_loader))

        with torch.no_grad():
            
            if self.gpu_mode:
                luma_in, chroma_in, ref_in = luma_in.to(self.device), chroma_in.to(self.device), ref_in.to(self.device)
            ...
            luma = self.build_fullSize_Pic(luma_in[:, 0, :, :].view(visual.__len__(), 32, 32))
            pre_Cb = self.build_fullSize_Pic(G_f[:, 0, :, :].view(visual.__len__(), 32, 32))
            pre_Cr = self.build_fullSize_Pic(G_f[:, 1, :, :].view(visual.__len__(), 32, 32))

            org_Cb = self.build_fullSize_Pic(org_label[:, 0, :, :].view(visual.__len__(), 32, 32))
            org_Cr = self.build_fullSize_Pic(org_label[:, 1, :, :].view(visual.__len__(), 32, 32))
    ...
    def build_fullSize_Pic(self, out_tensor, height = 1352 // 2, width = 1352 // 2):
